...
{	
    ...
    public function admin_login_check($data) {
        ...
        try
       {
          ...
          if($stmt->rowCount() > 0)
          {
			  ...
			  $message = ' Username ou senha invalido';
              return $message;
		  }
       }
    ...
    public function change_password_for_employee($data){
		...
		if($password == $re_password){
			...
			$message = 'Desculpe !! Senha invalida';
            return $message;
		}
	...
	public function add_new_user($data){
		...
		try{
			...
			if($total_email != 0 && $total_username != 0){
				$message = "Email e Password ja estão sendo usados";
            	return $message;

			}elseif($total_username != 0){
				$message = "Username ja esta sendo usado";
            	return $message;

			}elseif($total_email != 0){
				$message = "Email ja esta sendo usado";
            	return $message;
	...
	public function admin_password_change($data, $id){
		...
		try{
			...
			$all_error = '';
			if($total_row == 0){
				$all_error = "Senha antiga invalida";
			}
			

			if($admin_new_password != $admin_cnew_password ){
				$all_error .= '<br>'."Senha nova e confirmar senha nova não batem";
			}

			$password_length = strlen($admin_raw_password);

			if($password_length < 6){
				$all_error .= '<br>'."A senha deve ter mais de 6 caracteres";
			}
	...
	public function add_new_task($data){
		...
		try{
			...
			$_SESSION['Task_msg'] = 'Tarefa adicionada com sucessp';
		...
		public function update_task_info($data, $task_id, $user_role){
			...
			try{
				...
				$_SESSION['Task_msg'] = 'Tarefa atualizada com sucesso!';
